...
;;
;; Artificial action to enforce each day to be around 1440 minutes
;; (still can't model so that each day is exactly 1440 minutes)
;; changed days to be 960 minutes to make the problem a bit easier
(:durative-action initialize_day
 :parameters (?d0 ?d1 - Day)
 :duration (= ?duration 720)
 :condition (and 
	(over all (next ?d0 ?d1))
	
	(at start (initiated ?d0))
	(at start (uninitiated ?d1))
	)
:effect (and
	(at start (initiated ?d1))
	(at start (not (uninitiated ?d1)))
	(at end (not (initiated ?d1)))
	(at end (uninitiated ?d1))
	)
)

(:durative-action end_first_day
  	:parameters (?d - Day)
	:duration (= ?duration 10)
	:condition (at start (first_day ?d))
	:effect (and
		(at start (initiated ?d))
		(at end (and 
			(not (initiated ?d))
			(uninitiated ?d)
			(not (first_day ?d))	
...
(:durative-action sleep
 ...
 :condition (and 
	...
	(over all (initiated ?d))
)
 :effect (and (at start (not (available ?c)))
	   (at end (done_sleep ?c ?d)))
)
;; days end and start for a crew member when the post-sleep action runs
